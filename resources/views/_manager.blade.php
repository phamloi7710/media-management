...
<media-manager inline-template
               ...
               :config="{{ json_encode([
    'baseUrl' => $base_url,
    'hideFilesExt' => config('media.hide_files_ext'),
    'lazyLoad' => config('media.lazy_load_image_on_click'),
    'mimeTypes' => config('media.extended_mimes'),
    'cacheExp' => config('media.cache_expires_after'),
    'broadcasting' => config('media.enable_broadcasting'),
    'gfi' => config('media.get_folder_info'),
    'ratioBar' => config('media.show_ratio_bar'),
    'previewFilesBeforeUpload' => config('media.preview_files_before_upload')
    ]) }}"
               :routes="{{ json_encode([
    ...
    'copied' => trans('media::messages.copy.copied'),
    'copy_success' => trans('media::messages.copy.success'),
    'create_folder_notif' => trans('media::messages.new.create_folder_notif'),
    'create_success' => trans('media::messages.create_success'),
    'delete_success' => trans('media::messages.delete.success'),
    'downloaded' => trans('media::messages.download.downloaded'),
    'error_altered_fwli' => trans('media::messages.error.altered_fwli'),
    'find' => trans('media::messages.find'),
    'found' => trans('media::messages.found'),
    'glbl_search' => trans('media::messages.search.glbl'),
    'glbl_search_avail' => trans('media::messages.search.glbl_avail'),
    'go_to_folder' => trans('media::messages.go_to_folder'),
    'move_success' => trans('media::messages.move.success'),
    'new_uploads_notif' => trans('media::messages.upload.new_uploads_notif'),
    'no_val' => trans('media::messages.no_val'),
    'nothing_found' => trans('media::messages.nothing_found'),
    'refresh_notif' => trans('media::messages.refresh_notif'),
    'rename_success' => trans('media::messages.rename.success'),
    'save_success' => trans('media::messages.save.success'),
    'sep_download' => trans('media::messages.download.sep'),
    'stand_by' => trans('media::messages.stand_by'),
    'to_cp' => trans('media::messages.copy.to_cp'),
    'upload_success' => trans('media::messages.upload.success'),
    'upload_in_progress' => trans('media::messages.upload.in_progress'),
    'already_exists' => trans('media::messages.error.already_exists'),
    ]) }}"
               ...
               :user-id="{{ config('media.enable_broadcasting') ? auth()->user()->id : 0 }}"
    ...
    <div class="" :class="{'__stack-reverse': waitingForUpload}">
                        ...
                        <div class="field" :class="{'has-addons': !isBulkSelecting() && !restrictModeIsOn()}">
                            ...
                            <div class="control" v-if="!isBulkSelecting()">
                                <button class="button"
                                        ...
                                        v-tippy
                                        title="(U) {{ trans('media::messages.upload.main') }}">
                                    <span class="icon">
                            ...
                            <div class="control" v-if="!restrictModeIsOn()">
                                <button class="button"
                                        ...
                                        v-tippy
                                        title="{{ trans('media::messages.add.folder') }}">
                                    <span class="icon">
                        ...
                        <div class="field has-addons">
                            ...
                            <div class="control" v-if="!restrictModeIsOn()">
                                <button class="button is-link"
                                        ...
                                        v-tippy
                                        title="(M) {{ trans('media::messages.move.main') }}"
                                        @click="moveItem()">
                                    <span class="icon">
                            ...
                            <div class="control" v-if="!isBulkSelecting()">
                                <button class="button is-link"
                                        ...
                                        @click="renameItem()"
                                        title="{{ trans('media::messages.rename.main') }}">
                                    <span class="icon">
                            ...
                            <div class="control" v-show="!isBulkSelecting()">
                                <button class="button is-link"
                                        ...
                                        v-tippy
                                        title="(E) {{ trans('media::messages.editor.main') }}"
                                        @click="imageEditor()">
                                    <span class="icon">
                            ...
                            <div class="control">
                                <button class="button is-link"
                                        ...
                                        :disabled="item_ops() || isLoading"
                                        v-tippy
                                        title="(D / Del) {{ trans('media::messages.delete.main') }}"
                                        @click="deleteItem()">
                                    <span class="icon">
                        ...
                        <div class="field" :class="{'has-addons' : isBulkSelecting()}">
                            ...
                            <div class="control">
                                <button @click="blkSlctAll()"
                                    ...
                                    <template v-if="bulkSelectAll">
                                        ...
                                        <span>{{ trans('media::messages.select.non') }}</span>
                                    </template>
                                    <template v-else>
                                        <span class="icon">
                                            <icon name="plus" scale="0.8"></icon>
                                        </span>
                                        <span>{{ trans('media::messages.select.all') }}</span>
                                    </template>
                                </button>
                            </div>
                            {{-- bulk select --}}
                            <div class="control">
                                <button @click="blkSlct()"
                                    ...
                                    <span>{{ trans('media::messages.select.bulk') }}</span>
                                </button>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="field has-addons">
                                    <div class="control">
                                        <button @click="showFilesOfType('image')"
                                                v-tippy
                                                title="{{ trans('media::messages.filter_by', ['attr' => trans('media::messages.image')]) }}"
                                                class="button"
                                    ...
                                    <div class="control">
                                        <button @click="showFilesOfType('video')"
                                                v-tippy
                                                title="{{ trans('media::messages.filter_by', ['attr' => trans('media::messages.video')]) }}"
                                                class="button"
                                    ...
                                    <div class="control">
                                        <button @click="showFilesOfType('audio')"
                                                v-tippy
                                                title="{{ trans('media::messages.filter_by', ['attr' => trans('media::messages.audio.main')]) }}"
                                                class="button"
                                    ...
                                    <div class="control">
                                        <button @click="showFilesOfType('folder')"
                                                v-tippy
                                                title="{{ trans('media::messages.filter_by', ['attr' => trans('media::messages.folder')]) }}"
                                                class="button"
                                    ...
                                    <div class="control">
                                        <button @click="showFilesOfType('text')"
                                                v-tippy
                                                title="{{ trans('media::messages.filter_by', ['attr' => trans('media::messages.text')]) }}"
                                                class="button"
                                    ...
                                    <div class="control">
                                        <button @click="showFilesOfType('application')"
                                                v-tippy
                                                title="{{ trans('media::messages.filter_by', ['attr' => trans('media::messages.application')]) }}"
                                                class="button"
                                    ...
                                    <div class="control">
                                        <button @click="showFilesOfType('locked')"
                                                v-tippy
                                                title="{{ trans('media::messages.filter_by', ['attr' => trans('media::messages.locked')]) }}"
                                                class="button"
                                    ...
                                    <div class="control">
                                        <button @click="showFilesOfType('selected')"
                                                v-tippy
                                                title="{{ trans('media::messages.filter_by', ['attr' => trans('media::messages.select.selected')]) }}"
                                                class="button"
                                    ...
                                    <div class="control">
                                        <button @click="showFilesOfType('all')"
                                                v-tippy
                                                title="{{ trans('media::messages.clear', ['attr' => trans('media::messages.filter')]) }}"
                                                class="button"
                            ...
                            <div class="control has-icons-left">
                                <div class="select">
                                    <select v-model="sortBy" :disabled="isLoading">
                                        <option disabled value="null">{{ trans('media::messages.sort_by') }}</option>
                                        <option value="clear">{{ trans('media::messages.non') }}</option>
                                        <option value="size">{{ trans('media::messages.size') }}</option>
                                        <option value="last_modified">{{ trans('media::messages.last_modified') }}</option>
                                    </select>
                                </div>
                                ...
                                <div class="field has-addons">
                                    ...
                                    <p class="control">
                                        <button class="button is-black" :disabled="!searchFor"
                                                v-tippy
                                                title="{{ trans('media::messages.clear', ['attr' => trans('media::messages.search.main')]) }}"
        ...
        <section>
            <div class="media-manager__dz" :class="{'__dz-active': UploadArea}">
                <div id="new-upload" :style="uploadPanelImg">
                    {{-- text --}}
                    <div class="dz-message title is-4">{!! trans('media::messages.upload.text') !!}</div>
                    {{-- randomNames --}}
                    <div class="form-switcher"
                         title="{{ trans('media::messages.upload.use_random_names') }}"
                    ...
                    <div class="save_link" @click="toggleModal('save_link_modal')" v-if="!restrictUpload()">
                        <span class="icon is-large"
                              title="{{ trans('media::messages.save.link') }}"
        ...
        {{-- ====================================================================== --}}
        {{-- mobile breadCrumb --}}
        @include('media::partials.mobile-nav')
        {{-- ====================================================================== --}}
            ...
            <section class="__stack-container" :class="{'more-height': waitingForUpload}">
                ...
                <div id="uploadPreview">
                    <div class="dz-preview-ops">
                        ...
                        <button v-tippy="{arrow: true, hideOnClick: false}"
                                title="{{ trans('media::messages.add.more', ['attr' => null]) }}"
                                @click="toggleUploadPanel()"
                                class="btn-plain">
                            <span class="icon is-large">
                                <icon>
                        ...
                        {{-- upload --}}
                        <button v-tippy="{arrow: true, hideOnClick: false}"
                                title="{{ trans('media::messages.upload.main') }}"
                        ...
                        {{-- reset --}}
                        <button v-tippy="{arrow: true, hideOnClick: false}"
                                title="{{ trans('media::messages.clear', ['attr' => null]) }}"
                                ref="clear-dropzone"
                                class="btn-plain">
                            <span class="icon is-large">
                                <icon>
                ...
                <section>
                    ...
                    <div id="loading_files" v-show="loading_files">
                        <div id="loading_files_anim" data-json="{{ asset('app-assets/media/BM/world.json') }}"></div>
                        <transition name="mm-list" mode="out-in">
                            <h3 key="1" v-if="showProgress" class="mm-animated pulse">
                                {{ trans('media::messages.stand_by') }}
                                <strong>@{{ progressCounter }}</strong>
                            </h3>
                            <h3 key="2" v-else>{{ trans('media::messages.loading') }}</h3>
                        </transition>
                    </div>
                    {{-- ajax error --}}
                    <div id="ajax_error" v-show="ajax_error">
                        <div id="ajax_error_anim" data-json="{{ asset('app-assets/media/BM/avalanche.json') }}"></div>
                        <h3>{{ trans('media::messages.ajax_error') }}</h3>
                    </div>
                    {{-- no files --}}
                    <v-touch id="no_files"
                             v-show="no_files"
                        ...
                        <div id="no_files_anim" data-json="{{ asset('app-assets/media/BM/zero.json') }}"></div>
                        <h3>{{ trans('media::messages.no_files_in_folder') }}</h3>
                    </v-touch>
                    {{-- gesture --}}
                    <overlay></overlay>
                </section>
                ...
                <v-touch class="__stack-files mm-animated"
                    ...
                    <section>
                        <div id="no_search" v-show="no_search">
                            <div id="no_search_anim" data-json="{{ asset('app-assets/media/BM/ice_cream.json') }}"></div>
                        ...
                        <li v-for="(file, index) in orderBy(filterBy(allFiles, searchFor, 'name'), sortBy, -1)"
                            ...
                            <v-touch class="__file-box mm-animated"
                                ...
                                <button v-if="$refs.lock"
                                        ...
                                        :title="IsLocked(file) ? '{{ trans('media::messages.unlock') }}': '{{ trans('media::messages.lock') }}'"
                                ...
                                <div class="__box-data">
                                    ...
                                    <div class="__box-info">
                                        ...
                                        <template v-if="fileTypeIs(file, 'folder')">
                                            <h4>@{{ file.name }}</h4>
                                            <small>
                                                <span>@{{ file.count }} {{ trans('media::messages.items') }}</span>
                    ...
                    <div class="__sidebar-preview">
                        <transition name="mm-slide" mode="out-in" appear
                            ...
                            <div key="none-selected" class="__sidebar-none-selected" v-if="!selectedFile">
                                <span @click="reset()" class="link">
                                    <icon name="power-off" scale="3.2"></icon>
                                </span>
                                <p>{{ trans('media::messages.select.nothing') }}</p>
                            </div>
                            {{-- img --}}
                            <div v-else-if="selectedFileIs('image')"
                                ...
                                <transition :name="scrollableBtn.state ? 'mm-img-nxt': 'mm-img-prv'" appear>
                                    <div class="image-scroll-btn"
                                        ...
                                        <span class="icon is-large">
                                            <icon name="chevron-down" scale="1"></icon>
                                        </span>
                                    </div>
                                </transition>
                            </div>
                            ...
                            <div v-else-if="selectedFileIs('video')"
                                ...
                                <video controls
                                    ...
                                    {{ trans('media::messages.video_support') }}
                                </video>
                            </div>
                            {{-- audio --}}
                            <div v-else-if="selectedFileIs('audio')"
                                ...
                                <audio controls
                                       ...
                                       data-player
                                       :src="selectedFile.path">
                                    {{ trans('media::messages.audio.support') }}
                                </audio>
                            </div>
                            {{-- icons --}}
                            <icon key="1"
                    ...
                    <div v-if="allItemsCount"
                        ...
                        <transition name="mm-list" mode="out-in" appear>
                            <div :key="selectedFile.name" v-if="selectedFile">
                                ...
                                <table>
                                    <tr>
                                        <td class="t-key">{{ trans('media::messages.name') }}:</td>
                                        <td class="t-val">@{{ selectedFile.name }}</td>
                                    </tr>
                                </table>
                                <table>
                                    <tr>
                                        <td class="t-key">{{ trans('media::messages.type') }}:</td>
                                        <td class="t-val">@{{ selectedFile.type }}</td>
                                    </tr>
                                </table>
                                <table>
                                    <tr>
                                        <td class="t-key">{{ trans('media::messages.size') }}:</td>
                                ...
                                <template v-if="selectedFileIs('folder')">
                                    <table>
                                        <tr>
                                            <td class="t-key">{{ trans('media::messages.items') }}:</td>
                                    ...
                                    <div class="__sidebar-zip" v-show="!isBulkSelecting()">
                                        <table>
                                            <tr>
                                                <td class="t-key">{{ trans('media::messages.download.folder') }}:</td>
                                                <td class="t-val">
                                ...
                                <template v-else>
                                    <table v-if="(selectedFileIs('image') || selectedFileIs('video')) && dimensions.length">
                                        <tr>
                                            <td class="t-key">{{ trans('media::messages.dimension') }}:</td>
                                            <td class="t-val">@{{ selectedFileDimensions }}</td>
                                        </tr>
                                    </table>
                                    <table>
                                        <tr>
                                            <td class="t-key">{{ trans('media::messages.visibility.main') }}:</td>
                                            <td class="t-val">@{{ selectedFile.visibility }}</td>
                                        </tr>
                                    </table>
                                    <table>
                                        <tr>
                                            <td class="t-key">{{ trans('media::messages.preview') }}:</td>
                                            <td class="t-val"><a :href="selectedFile.path" target="_blank">{{ trans('media::messages.public_url') }}</a></td>
                                        </tr>
                                    </table>
                                    <div class="__sidebar-zip">
                                        <table>
                                            <tr>
                                                <td class="t-key">{{ trans('media::messages.download.file') }}:</td>
                                                <td class="t-val">
                                                    {{-- normal --}}
                                ...
                                <table>
                                    <tr>
                                        <td class="t-key">{{ trans('media::messages.last_modified') }}:</td>
                        ...
                        <transition-group tag="div" name="mm-counter" mode="out-in" class="__sidebar-count">
                            ...
                            <div key="1" v-if="allItemsCount">
                                ...
                                <p class="heading">{{ trans('media::messages.total') }}</p>
                            </div>
                            {{-- search --}}
                            ...
                            <div key="3" v-if="bulkItemsCount" class="__sidebar-count-bulk">
                                ...
                                <p class="heading">{{ trans('media::messages.select.selected') }}</p>
                            </div>
                        </transition-group>
                    </div>
                </v-touch>
            ...
            <section class="__stack-breadcrumb level is-mobile">
                            ...
                            <li key="library-bc">
                                <a v-if="folders.length > 0 && !(isBulkSelecting() || isLoading)"
                                   ...
                                   @click="goToFolder(0)">
                                    ...
                                    <span class="level-item m-l-5 is-marginless">{{ trans('media::messages.library') }}</span>
                                </a>
                ...
                <div class="level-right" v-if="waitingForUpload">
                    <nav class="breadcrumb">
                        <ul>
                            <li>
                                <p class="level has-text-weight-bold">@{{ selectedUploadPreviewList.length }} File's</p>
                            </li>
                            <li>
                        ...
                        <transition :name="infoSidebar ? 'mm-info-out' : 'mm-info-in'" mode="out-in">
                            <div :key="infoSidebar ? 1 : 2" class="__stack-sidebar-toggle has-text-link">
                                <template v-if="infoSidebar">
                                    <span>{{ trans('media::messages.close') }}</span>
                                    <span class="icon">
                                        <icon name="angle-double-right"></icon>
                                    </span>
                                </template>
                                <template v-else>
                                    <span>{{ trans('media::messages.open') }}</span>
                                    <span class="icon">
            ...
            <div v-if="isActiveModal('imageEditor_modal')"
                ...
                <div class="mm-animated fadeInDown __modal-content-wrapper">
                    <image-editor route="{{ route('media.uploadCropped') }}"
                                  ...
                                  :translations="{{ json_encode([
                        'clear' => trans('media::messages.clear', ['attr' => 'selection']),
                        'move' => trans('media::messages.move.main'),
                        'save_success' => trans('media::messages.save.success'),
                        'diff' => trans('media::messages.editor.diff'),
                        'presets' => trans('media::messages.crop.presets'),
                        'crop' => trans('media::messages.crop.main'),
                        'crop_reset' => trans('media::messages.crop.reset'),
                        'crop_reset_filters' => trans('media::messages.crop.reset_filters'),
                        'crop_apply' => trans('media::messages.crop.apply'),
                        'crop_zoom_in' => trans('media::messages.crop.zoom_in'),
                        'crop_zoom_out' => trans('media::messages.crop.zoom_out'),
                        'crop_rotate_left' => trans('media::messages.crop.rotate_left'),
                        'crop_rotate_right' => trans('media::messages.crop.rotate_right'),
                        'crop_flip_horizontal' => trans('media::messages.crop.flip_horizontal'),
                        'crop_flip_vertical' => trans('media::messages.crop.flip_vertical'),
                        ]) }}">
                    </image-editor>
                </div>
                <button class="modal-close is-large" @click="toggleModal()"></button>
                ...
                <div class="modal-card mm-animated fadeInDown">
                    <header class="modal-card-head is-warning">
                        <p class="modal-card-title">
                            ...
                            <transition name="mm-list" mode="out-in">
                                <span key="1" v-if="useCopy">{{ trans('media::messages.copy.file_folder') }}</span>
                    ...
                    <form action="{{ route('media.move_file') }}" @submit.prevent="MoveFileForm($event)">
                        <section class="modal-card-body">
                            {{-- destination --}}
                            <h5 class="subtitle m-b-10">{{ trans('media::messages.destination_folder') }}</h5>
                            <div class="field">
                                <div class="control has-icons-left">
                            ...
                            <br>
                            @include('media::partials.modal-files-info')
                        </section>
                        <footer class="modal-card-foot">
                            {{-- switcher --}}
                                    ...
                                    <div class="level-item">
                                        <button type="submit"
                                            ...
                                            <transition :name="useCopy ? 'mm-img-prv' : 'mm-img-nxt'" mode="out-in">
                                                <span key="1" v-if="useCopy">{{ trans('media::messages.copy.main') }}</span>
                                                <span key="2" v-else>{{ trans('media::messages.move.main') }}</span>
                                            </transition>
                                        </button>
                                    </div>
                                </div>
                    ...
                    <form action="{{ route('media.delete_file') }}"
                        ...
                        <section class="modal-card-body">
                            ...
                            <h5 v-show="folderWarning" class="__modal-folder-warning">
                                <span class="icon is-medium">
                                    <icon name="warning" scale="1.2"></icon>
                                </span>
...
{{-- styles --}}
@push('styles')
    <link rel="stylesheet" href="{{ asset('app-assets/media/style.css') }}"/>
@endpush
{{-- scripts --}}
@push('scripts')
