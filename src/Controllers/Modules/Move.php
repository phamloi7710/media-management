                ...
                if (!file_exists($new_path)) {
                        ...
                        if ($file_type == 'folder') {
                            ...
                            if (app('files')->copyDirectory($old, $new)) {
                                ...
                                $exc = isset($this->storageDiskInfo['root'])
                                        ? trans('media::messages.error.moving')
                                        : trans('media::messages.error.moving_cloud');
                    ...
                    // move
                        ...
                        if ($this->storageDisk->move($old_path, $new_path)) {
                            ...
                            $exc = trans('media::messages.error.moving');

                            if ($file_type == 'folder' && !isset($this->storageDiskInfo['root'])) {
                                $exc = trans('media::messages.error.moving_cloud');
                            }
                    ...
                    throw new Exception(
                        trans('media::messages.error.already_exists')
                    );
                }
            } catch (Exception $e) {
                $broadcast = false;
